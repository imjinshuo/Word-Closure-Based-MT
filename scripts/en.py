...
from utils.en_utils import *
from gensim import models
from sklearn.metrics.pairwise import cosine_similarity
w2v = models.KeyedVectors.load_word2vec_format('word2vec/merge_sgns_bigram_char300.txt.bz2', encoding='utf-8')
...
def zh_en_tokens(tokens):
    if_zh = False
    if_en = False
    for token in tokens:
        j = set(token.lower()).intersection(alpha)
        if j:
            if_en = True
        else:
            if_zh = True
...
def syn(token1_list, token2_list):
    ...
    if token1_list == token2_list:
        return 1.0
    token1_list = [''.join(token1_list)]
    token2_list = [''.join(token2_list)]
    for token1 in token1_list:
        this_syn = get_syn('zh', token1)
        s_syn.extend(this_syn)
    for token2 in token2_list:
        this_syn = get_syn('zh', token2)
        f_syn.extend(this_syn)
    if set(token1_list).issubset(set(f_syn)) or set(token2_list).issubset(set(s_syn)):
        return 1.0
    else:
        return 0.0
...
def exp(file, save_file, threshold, tokenizer, config=13, clo='WordClosure', IT='', opt=True, sem=True, disable_print=False):
    ...
    for i in tqdm(i_index_list):
        ...
        diff_zh_s_tokens = [zh_s_tokens[id] for id in sim_reached_uncompared_idx_s]
        diff_zh_f_tokens = [zh_f_tokens[id] for id in sim_reached_uncompared_idx_f]
            ...
            if s_diff_words_ids and f_diff_words_ids:
                if sem:
                    if config == 1:
                        ...
                        this_diff_score = sim_bert('zh', zh_s_tokens, zh_f_tokens, s_diff_words_ids, f_diff_words_ids)
                    elif config == 12:
                        this_diff_score = w2v_sim(s_diff_words, f_diff_words)
                        if syn(s_diff_words, f_diff_words):
                            this_diff_score = 1.0
        ...
        for this_closureid, this_closure in enumerate(closures_for_comparison):
            ...
            if sem:
                if config == 1:
                    ...
                    this_score = sim_bert('zh', zh_s_tokens, zh_f_tokens, this_closure[2], this_closure[3])
                elif config == 12:
                    this_score = w2v_sim(s_pair_tokens, f_pair_tokens)
                    if syn(s_pair_tokens, f_pair_tokens):
                        this_score = 1.0
                    ...
                    if syn(s_pair_tokens, f_pair_tokens):
                        this_score = 1.0
                if zh_en_tokens(s_pair_tokens) != zh_en_tokens(f_pair_tokens):
                    this_score = 0.0
            else:
                if set(s_pair_tokens) == set(f_pair_tokens):
                    this_score = 1.0
            if is_STOP(s_pair_tokens, 'zh') and is_STOP(f_pair_tokens, 'zh'):
                this_score = 1.0
        ...
        not_included_zh_s_idx = [id for id in not_included_idx_s if zh_s_tokens[id] not in punc
                                 if not is_STOP(zh_s_tokens[id], 'zh')
                                 and zh_s_tokens[id] not in repeated_tokens_s and zh_s_tokens[id] not in trans_f_line]# and zh_s_tokens[id] not in STOP_align_tokens_s]
        not_included_zh_f_idx = [id for id in not_included_idx_f if zh_f_tokens[id] not in punc
                                 if not is_STOP(zh_f_tokens[id], 'zh')
                                 and zh_f_tokens[id] not in repeated_tokens_f and zh_f_tokens[id] not in trans_s_line]# and zh_f_tokens[id] not in STOP_align_tokens_f]
                ...
                for token2, idx2 in zip(not_included_tokens_f, not_included_zh_f_idx):
                    ...
                    if sem:
                        if config == 1:
                            ...
                            same_score = sim_bert('zh', zh_s_tokens, zh_f_tokens, [idx1], [idx2])
                        elif config == 12:
                            same_score = w2v_sim([token1], [token2])
                            if syn([token1], [token2]):
                                same_score = 1.0
        ...
        if IT == 'PatInv':
            print(f"{color.BOLD}MWC{color.END}: {color.BOLD}{color.BLUE}{diff_zh_s_tokens}, {diff_zh_f_tokens} (similarity:{this_diff_score}){color.END}")
            if this_diff_score > dissim_threshold:
                print(f"{color.BOLD}Violated MWC{color.END}: {color.BOLD}{color.RED}{diff_zh_s_tokens}, {diff_zh_f_tokens} (similarity:{this_diff_score}){color.END}")
