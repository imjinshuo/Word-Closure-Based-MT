...
def is_STOP(token, lang):
    if lang == 'zh':
        ...
        if type(token) == list:
            ...
            if stemmer.stem(token.lower()) in en_stopwords or token.lower() in punc or token.lower() in en_stopwords or lemmatizer.lemmatize(token.lower()) in en_stopwords:
                return True
            else:
                return False
...
def optimize_align(s_align, f_align, same_index_s, diff_index_source, diff_index_follow, out_source2follow, source2follow, follow2source, s_align_source, f_align_source, en_s_tokens, en_f_tokens, zh_s_tokens, zh_f_tokens):
    ...
    s_zh_keys.sort()
    f_zh_keys.sort()
    zh_s_tokens_lemma = [lemmatizer.lemmatize(token.lower()) for token in zh_s_tokens]
    zh_f_tokens_lemma = [lemmatizer.lemmatize(token.lower()) for token in zh_f_tokens]
    notalign_id_s = [id for id in range(len(zh_s_tokens)) if id not in s_zh_keys and not is_PUNCT(zh_s_tokens[id], 'en')]
    notalign_id_f = [id for id in range(len(zh_f_tokens)) if id not in f_zh_keys and not is_PUNCT(zh_f_tokens[id], 'en')]
    phrase_tags = ['ADJP', 'ADVP', 'CONJP', 'FRAG', 'INTJ', 'NAC', 'NP', 'NX', 'PP', 'PRN', 'QP', 'RRC', 'UCP', 'WHADJP', 'WHADV', 'WHNP', 'WHPP', 'X']
    if notalign_id_s or notalign_id_f:
        corenlp_s = nlp_en.annotate(' '.join(zh_s_tokens), properties={
            'annotators': 'pos,depparse,parse',
            'tokenize.whitespace': 'true',
            'ssplit.eolonly': 'true',
            'outputFormat': 'json'
        })
        corenlp_f = nlp_en.annotate(' '.join(zh_f_tokens), properties={
            'annotators': 'pos,depparse,parse',
            'tokenize.whitespace': 'true',
            'ssplit.eolonly': 'true',
            'outputFormat': 'json'
        ...
        for id in notalign_id_s:
            ...
            if not neig_ids and len(this_aligned_leaves) == 1:
                if zh_s_tokens_lemma[id] in zh_f_tokens_lemma and is_STOP(zh_s_tokens[id], 'en'):
                    continue
                ...
                if if_diff:
                    ...
                    for key in add_source_id:
                        if id not in s_map_sf[key]:
                            s_map_sf[key].append(id)
                            s_zh_keys.append(id)
                elif this_sub_tree._label in phrase_tags or (len(this_aligned_leaves) == 1 and zh_s_tokens_lemma[id] not in zh_f_tokens_lemma):
                    if is_STOP(zh_s_tokens[id], 'en') and len(this_leaves) != 2:
                        continue
                    for key in add_source_id:
                        if id not in s_map_sf[key]:
                            s_map_sf[key].append(id)
                ...
                if neig_ids[0] in diff_2out_id_s:
                    ...
                    for key in add_source_id:
                        if id not in s_map_sf[key]:
                            s_map_sf[key].append(id)
                            s_zh_keys.append(id)
                elif zh_s_tokens_lemma[neig_ids[0]] not in ' '.join(zh_f_tokens_lemma) and zh_s_tokens_lemma[neig_ids[1]] in ' '.join(zh_f_tokens_lemma):
                    if this_sub_tree._label in phrase_tags or (len(this_aligned_leaves) == 1 and zh_s_tokens_lemma[id] not in zh_f_tokens_lemma):
                        if is_STOP(zh_s_tokens[id], 'en') and len(this_leaves) != 2:
                            continue
                        target_neig = neig_ids[0]
                        add_source_id = []
                        ...
                        for key in add_source_id:
                            if id not in s_map_sf[key]:
                                s_map_sf[key].append(id)
                                s_zh_keys.append(id)
                elif zh_s_tokens_lemma[neig_ids[0]] in ' '.join(zh_f_tokens_lemma) and zh_s_tokens_lemma[neig_ids[1]] not in ' '.join(zh_f_tokens_lemma):
                    if this_sub_tree._label in phrase_tags or (len(this_aligned_leaves) == 1 and zh_s_tokens_lemma[id] not in zh_f_tokens_lemma):
                        if is_STOP(zh_s_tokens[id], 'en') and len(this_leaves) != 2:
                            continue
                        target_neig = neig_ids[1]
        ...
        for id in notalign_id_f:
            ...
            if not neig_ids and len(this_aligned_leaves) == 1:
                if zh_f_tokens_lemma[id] in zh_s_tokens_lemma and is_STOP(zh_f_tokens[id], 'en'):
                    continue
                target_neig = this_aligned_leaves[0]
                add_source_id = []
                for key, val in f_map_sf.items():
                ...
                if if_diff:
                    ...
                    for key in add_source_id:
                        if id not in f_map_sf[key]:
                            f_map_sf[key].append(id)
                            f_zh_keys.append(id)
                elif this_sub_tree._label in phrase_tags or (len(this_aligned_leaves) == 1 and zh_f_tokens_lemma[id] not in zh_s_tokens_lemma):
                    if is_STOP(zh_f_tokens[id], 'en') and len(this_leaves) != 2:
                        continue
                    for key in add_source_id:
                        if id not in f_map_sf[key]:
                            f_map_sf[key].append(id)
                ...
                if neig_ids[0] in diff_2out_id_f:
                    ...
                    for key in add_source_id:
                        if id not in f_map_sf[key]:
                            f_map_sf[key].append(id)
                            f_zh_keys.append(id)
                elif zh_f_tokens_lemma[neig_ids[0]] not in ' '.join(zh_s_tokens_lemma) and zh_f_tokens_lemma[neig_ids[1]] in ' '.join(zh_s_tokens_lemma):
                    if this_sub_tree._label in phrase_tags or (len(this_aligned_leaves) == 1 and zh_f_tokens_lemma[id] not in zh_s_tokens_lemma):
                        if is_STOP(zh_f_tokens[id], 'en') and len(this_leaves) != 2:
                            continue
                        target_neig = neig_ids[0]
                        add_source_id = []
                        ...
                        for key in add_source_id:
                            if id not in f_map_sf[key]:
                                f_map_sf[key].append(id)
                                f_zh_keys.append(id)
                elif zh_f_tokens_lemma[neig_ids[0]] in ' '.join(zh_s_tokens_lemma) and zh_f_tokens_lemma[neig_ids[1]] not in ' '.join(zh_s_tokens_lemma):
                    if this_sub_tree._label in phrase_tags or (len(this_aligned_leaves) == 1 and zh_f_tokens_lemma[id] not in zh_s_tokens_lemma):
                        if is_STOP(zh_f_tokens[id], 'en') and len(this_leaves) != 2:
                            continue
                        target_neig = neig_ids[1]
...
def get_neighbour(id, this_aligned_leaves, s_tokens):
    neig_ids = []
    if id - 1 in this_aligned_leaves and not is_STOP(s_tokens[id-1], 'en'):
        neig_ids.append(id - 1)
    elif id - 2 in this_aligned_leaves and not is_STOP(s_tokens[id-2], 'en'):
        neig_ids.append(id - 2)
    elif id - 3 in this_aligned_leaves and not is_STOP(s_tokens[id-3], 'en'):
...
def get_phrase(tokens):
    corenlp = nlp_en.annotate(' '.join(tokens), properties={
        'annotators': 'pos,depparse,parse',
        'tokenize.whitespace': 'true',
        'ssplit.eolonly': 'true',
        'outputFormat': 'json'
...
def clause_traverse(tree, phrases, phrase):
    for idx, subtree in enumerate(tree):
        ...
        if type(subtree) == nltk.tree.Tree:
            if subtree._label in ['S', 'SBAR', 'RRC', 'SBARQ', 'SQ']:
                phrases.append(this_phrase)
...
def align(s_line, f_line, trans_s_line, trans_f_line, tokenizer, clo=True, opt=True):
    s_align_source, en_s_tokens, zh_s_tokens = word_align('zh2en', s_line, trans_s_line, tokenizer, tokenized=False,
                                                   if_punc=True, if_stop=False)
    f_align_source, en_f_tokens, zh_f_tokens = word_align('zh2en', f_line, trans_f_line, tokenizer, tokenized=False,
                                                   if_punc=True, if_stop=False)
    ...
    same_index_source, same_index_follow, diff_index_source, diff_index_follow, source2follow, follow2source = findDifference(
    ...
    if clo == 'WordClosure':
        ...
        for index in s_indexes:
            this_closure = buildClosure([[index], [], [], []], s_map, s_map_reverse, f_map, f_map_reverse, source2follow,
                                        follow2source)
            ...
            if idx_s in s_map_reverse and idx_s not in closures_not_idx_s:
                for idx_f, unit_f in enumerate(zh_f_tokens):
                    if idx_f in f_map_reverse and idx_f not in closures_not_idx_f:
                        ...
                        aligned_ids_s2f = [source2follow[this_s_aligned][0] for this_s_aligned in aligned_ids_s
                                           if this_s_aligned in source2follow]
